...
img_mean_np = np.array([0.485, 0.456, 0.406])
img_std_np = np.array([0.229, 0.224, 0.225])
img_mean = torch.from_numpy(img_mean_np).float().to(cfg.device)
img_std = torch.from_numpy(img_std_np).float().to(cfg.device)


def process_bbox(left, top, width, height, scale_width=1.5, scale_height=1.5):
    new_size = int(max(width * scale_width, height * scale_height))
...
def crop_and_resize(img, bb, crop_size, rgb=True):
    crop_transform = transforms.functional.resized_crop
    img = torch.from_numpy(img).float().to(cfg.device)
    if rgb:
        img = crop_transform(img.permute(2,0,1)[None], bb[1], bb[0], bb[3], bb[2], crop_size)
    else:
        img = crop_transform(img[None,None], bb[1], bb[0], bb[3], bb[2], crop_size, transforms.InterpolationMode.NEAREST)
    return img
...
def save_img(out_file, img):
    if out_file.endswith('.gif'):
        imageio.mimsave(osp.join(cfg.output_dir, out_file), img)
    else:
        img = np.concatenate([img[:,:,2::-1], img[:,:,3:]], 2)
        cv2.imwrite(osp.join(cfg.output_dir, out_file), img)
