...
def apply_crf(unary, img):
    ...
    compat = [50,15]
    img = np.ascontiguousarray((img * img_std_np + img_mean_np).clip(0,1)*255.)
...
def cluster_features(features, saliency_maps, images, masks_fg=None, crf=True):
    h, w = images[0].shape[2]//cfg.ps, images[0].shape[3]//cfg.ps
    ...
    for i, (k, img) in enumerate(zip(features, images)):
        ...
        if crf:
            ...
            unary = torch.from_numpy(unary)[None].to(cfg.device)
            unary = F.interpolate(unary, img.shape[-2:], mode='bilinear', align_corners=False)
            mask = apply_crf(unary[0].cpu().numpy(), img[0].permute(1,2,0).cpu().numpy())
            if masks_fg is not None:
                mask *= masks_fg[i][0,0].cpu().numpy().astype(np.int64)
        else:
            mask = labels.copy()+1 
            mask[~is_fg] = 0           
            mask = torch.from_numpy(mask.reshape(h, w))[None,None].float().to(cfg.device)
            mask = F.interpolate(mask, cfg.input_size, mode='nearest')[0,0].cpu().numpy()
    ...
    centroids = torch.from_numpy(centroids).to(cfg.device)        
